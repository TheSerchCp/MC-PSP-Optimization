...
#include <Minecraft/Screens/StateOptions.h>
#include <Minecraft/Player/State/ManagePlayerState.h>
...
void StateMenu::Init()
{
    ...
    lamecraftSprite = new Sprite(TextureHelper::Instance()->GetTexture(TextureHelper::Minecraft),0,0,320,90);
                ...
                if(loadSelectPos == 0)//play selected world
                {
                    if(saveFilesList.size() > 0)
                    {
                        if(saveFilesList[loadSavePos].locked == false)
                        {
                            ...
                            ManagePlayerState *statePlay = new ManagePlayerState();
                            char gameMode =  saveFilesList[loadSavePos].worldGameMode == SURVIVAL ? 0 : saveFilesList[loadSavePos].worldGameMode == HARDCORE ? 2 : 1;
                            statePlay->Init(gameMode);
                            statePlay->LoadMap(saveFilesList[loadSavePos].fileName,saveFilesList[loadSavePos].compression,gameMode);
                            statePlay->InitCamera();
                            sManager->PushState(statePlay);
                            // if(saveFilesList[loadSavePos].worldGameMode == SURVIVAL || saveFilesList[loadSavePos].worldGameMode == HARDCORE)
                            // {
        ...
        if(mSystemMgr->KeyPressed(PSP_CTRL_CROSS))
        {
            ...
            if(converterPos == 2)
            {
				...
				if(newW_deleteCaves == true)
				{
                    ...
                    for(int o = 0; o <= 35; o += 1) // Èíèöèàëèçàöèÿ èíâåíòàðÿ
                    {
                        invAm[o] = -1; // Êîëè÷åñòâî âåùåé â ÿ÷åéêè = -1 (0)
                        invId[o] = -1; // Id âåùåé â ÿ÷åéêå = -1 (ÿ÷åéêà ïóñòà)
                        invSt[o] = false; // Âåùü â ÿ÷åéêå íå ñòàêàåòñÿ (ïî óìîë÷àíèþ)
                    }
...
void StateMenu::Draw(StateManager* sManager)
{
    ...
    if (background == true)
    {
        ...
        DrawText(15,270,GU_COLOR(1,1,1,1) ,0.35,"Cophyright By Regen Studio Devs");
		DrawText(415,270,GU_COLOR(1,1,1,1) ,0.35,"Minecraft PSP BETA");
		...
		if(RenderManager::InstancePtr()->GetFontLanguage() == SPANISH)
        {
...
// void StateMenu::DrawTextLabel(int posX, int posY, const char *label, bool isSelected)
// {
//     sceGuDisable(GU_DEPTH_TEST);
//     sceGuEnable(GU_BLEND);
//     if (isSelected)
