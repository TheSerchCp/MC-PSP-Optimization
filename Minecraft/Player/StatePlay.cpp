                ...
                for(float i = 0; i < 5.25f; i+=0.1f)
                {
                    ...
                    if(mWorld->BlockEditable(testPos.x,testPos.y,testPos.z) && testBox.contains(testPos))
                    {
                        ...
                        if(selectedBlock >= BedBlock1::getID() && selectedBlock <= BedBlock8::getID() && sleepMessageTime <= 0.0f)
                        {
                            ...
                            {
                                for(int f = 0; f < mWorld->mEndermans.size(); f++)
                                {
                                    Enderman *TestEnderman = mWorld->mEndermans[f];
                                    if(abs(playerPosition.x-TestEnderman->position.x) <= 7 && abs(playerPosition.z-TestEnderman->position.z) <= 7 && abs(playerPosition.y-TestEnderman->position.y) <= 4)
                                    {
                                        sleepMessageTime = 3.0f;
                                        sleepMessageType = 2;
                                        break;
                                   ...
                                   mWorld->invId[27+barPosition] == Potato::getID())
                                {
                                    ...
                                    if(mWorld->CanPutBlockHere(testPos2.x,testPos2.y,testPos2.z,mWorld->invId[27+barPosition]))
                                    {
                                        ...
                                        else //if it is item in our hand
                                        {
                                            ...
                                            if(mWorld->invId[27+barPosition] == EndermanEGG::getID())
											{
													mWorld->SpawnEnderman(testPos2.x,testPos2.y,testPos2.z);
...
void StatePlay::Draw(StateManager* sManager)
{
    ...
    if(mWorld->mEndermans.empty() == false)
    {
          for(int d = 0; d < mWorld->mEndermans.size(); d++)
        {
            if(d < mWorld->mEndermans.size() && mWorld->mEndermans.empty() == false)
            {
                Enderman* TestEnderman = mWorld->mEndermans[d];

                if(menuState == 0)//game state
                {
                    TestEnderman->Update(mWorld, playerPosition, dt);
                }
                TestEnderman->Render(fppCam->mFrustum, dt);

                if(TestEnderman->playHurtSound)
                {
                    mSoundMgr->PlayZombieHurtSound(TestEnderman->FastDistanceToPlayer());
                    TestEnderman->playHurtSound = false;
                }

                if(TestEnderman->playSaySound)
                {
                    mSoundMgr->PlayZombieSaySound(TestEnderman->FastDistanceToPlayer());
                    TestEnderman->playSaySound = false;
                }

                if(TestEnderman->killed == true)
                {
                    if(TestEnderman->playDieSound)
                    {
                        mSoundMgr->PlayZombieDieSound(TestEnderman->FastDistanceToPlayer());
                        TestEnderman->playDieSound = false;
                    }
                }

                if(TestEnderman->toDestroy == true || (TestEnderman->killed == true && TestEnderman->deathTimer <= 0.0f))
                {
                    if(TestEnderman->killed == true)
                    {
                        if(TestEnderman->lastKickType != 2) // kicked by player or other death
                        {
                            int rfAmount = rand() % 3;
                            if(rfAmount != 0)
                            {
                                mWorld->DropThis(RottenFlesh::getID(),rfAmount,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            }

                            // rare drop , solo dejar enderpearl
                            if(rand() % 100 <= 2)
                            {
                                mWorld->DropThis(IronBar::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            }
                            if(rand() % 100 <= 3)
                            {
                                mWorld->DropThis(Potato::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            }
                            if(rand() % 100 <= 3)
                            {
                                mWorld->DropThis(Carrot::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            }
                            // end
                        ...
                        {
                            mWorld->DropThis(Bone::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            mWorld->DropThis(Bone::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                            if(rand()%3 == 0)
                            {
                                mWorld->DropThis(Bone::getID(),1,true,Vector3(TestEnderman->position.x,TestEnderman->position.y+0.3f,TestEnderman->position.z));
                    ...
                    for(int k = 0; k <= 7; k++)
                    {
                        Particle* Smoke = new Particle(mWorld, "smoke",Vector3(TestEnderman->position.x-0.4+(rand()%9/10.0f),TestEnderman->position.y+0.2f+(rand()%7/5.0f),TestEnderman->position.z-0.4+(rand()%9/10.0f)));
                        mParticles.push_back(Smoke);
                    }


                    mWorld->DestroyEnderman(d);
					if(TestEnderman->killed == true && TestEnderman->lastKickType == 1)
					{
